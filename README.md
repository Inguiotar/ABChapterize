...
| `-j`, `--mark-before-jingle` | **Experimental.** Walk the mark backward from the default placement to the end of the previous chapter's actual narration, through any silence and jingle music in between, instead of the default fixed offset before the phrase (see [How it works](#how-it-works)). Combinable with `-p`, which then supplies the starting point. |
