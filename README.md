...
[Releases](../../releases) page — no build tools required. The full
documentation is online at
**[abchapterize.anetos.de](https://abchapterize.anetos.de)**, published with
each release.
...
- **Detects the language automatically** — by default, each file's language
  ...
  language with `--lang de` if you'd rather skip per-file detection, or name
  several — `--lang no,da,sv` — to keep the per-file detection but let it
  choose only between those. A file that fits none of them falls back to the
  first language you named rather than to English, so put your shelf's most
  likely language first.
...
when chapters are written. The same manual is at
[abchapterize.anetos.de](https://abchapterize.anetos.de), split into pages with
a navigation column — the friendlier way to read a document this long. Grouped
below exactly as `--help` groups them:
...
| `--abs-push` | Mark local files as usual — writing the marks into each file — and send the finished list to the server as well. Each file is matched to the server's libraries as below; one that matches nothing is written anyway and simply not sent. A set left with a chapter-sequence gap is sent only where the server has fewer marks for that book, so an unmarked book gets the partial set while a longer list on the server is left alone — unless `--max-chapters` condemns that list as bogus, in which case it is replaced anyway. Not combinable with `--abs`. |
| `--abs-push-only` | Send the server the marks a book already carries, detecting nothing. With `--abs` the selected books are fetched and read; without it, the local files named are matched to the server's libraries by their album tag, title tag, folder name or file name — and, where none of those names a book, by the name the server holds the book's own audio file under. |
| `--abs-map <path>` | Say which Audiobookshelf book a local file is a copy of, for the files nothing about them names one for. One `<file name> = <book>` entry per line, where the book is `item:ID` or a title, and `none` (or nothing) marks a file the server has no book for. A folder may carry its own as `.abchapterize-abs`. An entry says which book, not that the two are the same recording: the play-time test still applies. See the [manual](doc/manual.md#saying-which-book-a-file-is). |
...
| Option | What it does |
| --- | --- |
| `-l`, `--lang <codes\|auto>` | Language hint for Whisper, or `auto` (the default): each file's language is detected from short samples taken inside the book — up to five, from different places, until one is confident or they can be voted on — and used for that file, falling back to `en` only when they cannot agree. Several codes separated by commas (`-l no,da,sv`) keep that per-file detection but let it choose only between those languages; a file fitting none of them falls back to the **first** one named rather than to `en`, so order matters. Numbers transcribed as words — cardinal and ordinal, before or after the phrase — are understood in `en`, `de`, `fr`, `es`, `it`, `nl`, `tr`, `pt`, `pl`, `sv`, `da`, `no`, `cs`; digits (`12`, `2nd`, `2e`) and Roman numerals (`XIII`) in every language. Also localizes the defaults of `--chapter-phrase`, `--prologue-phrase`, `--epilogue-phrase`, `--chapter-title`, `--part-title`, `--intro-title`, `--prologue-title` and `--epilogue-title` (per-file with `auto`). |
...
| `--no-rename` | Never tag a file name with `.missing-marks-…`. A file left with a chapter-sequence gap still gets the marks that were found and is still reported as incomplete; it simply keeps its own name — for a library whose names belong to a media server, a torrent client or somebody else's script. The cost is the automatic resume: the tag is what tells a later run which chapters to go back for, so such a file is skipped next time for the marks it already carries and finishing it needs `--force`. A tag an earlier run left behind is still taken off once the file is complete, which gives the file its own name back rather than imposing one. |
...
- **`Probe`** — a short stretch of audio is transcribed with Whisper at
   ...
   that crosses one.
- **`J-probe` and `S-probe`** — a book that announces every
...
- **`SD-probe`** — a book with *no* chapter music has the same
