...
/// from the probe transcriber except by reference - which is exactly the check it uses to decide
/// whether a separate upgrade model exists at all.
/// </para>
/// <para>
...
/// </para>
/// </summary>
public sealed class UpgradeTranscriber : ITranscriber, IAsyncDisposable
{
    ...
    /// <summary>Creates the upgrade transcriber for the given model, without loading it yet.</summary>
    /// <param name="model">Validated upgrade model selector (see <see cref="CliOptions.UpgradeModel"/>).</param>
    ...
    /// have used.</param>
    /// <param name="threads">CPU threads for this model, the same budget Probe gets
    ...
    /// <param name="gpuDevice">Forwarded likewise, so Scan lands on the same GPU the rest of the
    ...
    public UpgradeTranscriber(string model, string language, int threads, bool forceCpu = false,
    ...
    /// <summary>
    /// Not supported: language auto-detection happens once per file on a Probe window, never on the
    /// upgrade model. <see cref="ChapterDetector"/> only ever calls this on its probe transcriber, so
